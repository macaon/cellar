...
# Installer executable names, in priority order (case-insensitive).
_INSTALLER_NAMES = (
    "INSTALL.EXE", "INSTALL.COM", "INSTALL.BAT",
    "SETUP.EXE", "SETUP.COM", "SETUP.BAT",
)
...
def scan_iso(path: Path) -> list[str]:
    """List all files on an ISO 9660 image.

    Returns file paths as strings (e.g. ``"/INSTALL.EXE"``, ``"/GAME/GAME.EXE"``).
    Tries Joliet extensions first, falls back to plain ISO 9660.
    Old DOS ISOs often have non-standard headers, so we retry with relaxed
    parsing if strict mode fails.
    """
    import pycdlib

    iso = pycdlib.PyCdlib()

    # Try strict mode first, fall back to relaxed for non-compliant ISOs.
    try:
        iso.open(str(path))
    except Exception:
        iso = pycdlib.PyCdlib()
        try:
            iso.open(str(path), "rb")
        except Exception:
            log.warning("pycdlib cannot open ISO %s, trying isoinfo fallback", path)
            return _scan_iso_fallback(path)

    try:
        return _walk_iso(iso)
    except Exception:
        log.warning("pycdlib walk failed for %s, trying isoinfo fallback", path)
        return _scan_iso_fallback(path)
    finally:
        iso.close()


def _walk_iso(iso) -> list[str]:
    """Walk an opened PyCdlib object and return all file paths."""
    import pycdlib

    result: list[str] = []

    # Try Joliet first (supports long filenames).
    try:
        for dirpath, _dirnames, filenames in iso.walk(joliet_path="/"):
    ...
    # Fall back to plain ISO 9660.
    for dirpath, _dirnames, filenames in iso.walk(iso_path="/"):
        for fname in filenames:
            full = dirpath.rstrip("/") + "/" + fname
            # ISO 9660 Level 1 appends ";1" version suffix
            if ";" in full:
                full = full.split(";")[0]
            result.append(full)
...
def _scan_iso_fallback(path: Path) -> list[str]:
    """Fallback ISO scanner using isoinfo CLI or raw PVD reading.

    Used when pycdlib fails on non-compliant ISOs (common with old DOS games).
    """
    import shutil as _shutil

    # Try isoinfo (from cdrtools/genisoimage) if available.
    isoinfo = _shutil.which("isoinfo")
    if isoinfo:
        try:
            result = subprocess.run(
                [isoinfo, "-l", "-i", str(path)],
            ...
            if result.returncode == 0:
                ...
                for line in result.stdout.splitlines():
                    ...
                    if line.startswith("Directory listing of "):
                        current_dir = line.split("Directory listing of ", 1)[1].strip()
                    elif line and not line.startswith("d") and "." in line:
                        # File entry — extract filename (last column after whitespace)
                        parts = line.split()
                        if parts:
                            fname = parts[-1].rstrip(";1")
                            files.append(current_dir.rstrip("/") + "/" + fname)
                if files:
                    return files
        except Exception as exc:
            log.debug("isoinfo fallback failed: %s", exc)
...
def find_dos_installer(file_list: list[str]) -> str | None:
    """Find a DOS installer executable in a list of file paths.

    Returns the best match by priority, or ``None`` if no installer found.
    File paths should be absolute (e.g. ``"/INSTALL.EXE"``).
    """
    # Build lookup: uppercase basename → full path
    by_name: dict[str, list[str]] = {}
    for fp in file_list:
        name = Path(fp).name.upper()
        by_name.setdefault(name, []).append(fp)

    for installer_name in _INSTALLER_NAMES:
        matches = by_name.get(installer_name, [])
        if matches:
            return matches[0]
