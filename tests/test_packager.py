...
def test_compress_directory_zst_roundtrip(tmp_path):
    """Compressed archive contains all source files with correct content."""
    src = tmp_path / "mygame"
    src.mkdir()
    (src / "mygame.x86_64").write_bytes(b"binary")
    (src / "data").mkdir()
    ...
    assert "mygame/mygame.x86_64" in names
    assert "mygame/data/assets.pak" in names


def test_compress_directory_zst_cancelled(tmp_path):
    """CancelledError is raised when cancel_event is set before compression."""
    import threading
    src = tmp_path / "app"
    src.mkdir()
    (src / "game").write_bytes(b"x" * 1024)

    dest = tmp_path / "app.tar.zst"
    ev = threading.Event()
    ev.set()

    with pytest.raises(pkg.CancelledError):
        pkg.compress_directory_zst(src, dest, cancel_event=ev)

    assert not dest.exists()


def test_compress_directory_zst_progress(tmp_path):
    """progress_cb is called at least once with a value in [0, 1]."""
    src = tmp_path / "app"
    src.mkdir()
    (src / "a").write_bytes(b"aaa")
    (src / "b").write_bytes(b"bbb")

    dest = tmp_path / "app.tar.zst"
    calls: list[float] = []
    pkg.compress_directory_zst(src, dest, progress_cb=calls.append)
